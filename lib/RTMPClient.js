...
class RTMPClient {
  ...
  async push (videoFilePath) {
    ...
    let ffmpegCLI = ffmpeg(videoFilePath)
    ...
    ffmpegCLI = ffmpegCLI.videoFilters([{
      filter: 'drawtext',
      options: `fontfile="./lib/msyh.ttc":text="${path.basename(videoFilePath)}":fontcolor=black:fontsize=16:box=1:boxcolor=white@0.2:boxborderw=5:x=5:y=5`
    }])

    ffmpegCLI = ffmpegCLI.output(outputPath)
    return new Promise((resolve, reject) => {
      ffmpegCLI.on('start', function (commandLine) {
        logger.info(`Spawned Ffmpeg with command: ${commandLine}`)
        logger.info(`Video ${videoFilePath} is Pushing `)
        logger.info(`Spawned Ffmpeg with command !`)
      })
