...
class RTMPClient {
  ...
  async push (videoFilePath) {
    ...
    ffmpegCLI = ffmpegCLI.videoFilters([{
      filter: 'drawtext',
      options: `fontfile="${path.join(__dirname, './msyh.ttc')}":text="${path.basename(videoFilePath)}":fontcolor=black:fontsize=16:box=1:boxcolor=white@0.2:boxborderw=5:x=5:y=5`
    }])
