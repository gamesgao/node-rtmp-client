...
class RTMPClient {
  ...
  async pushFolder (videoFolderPath, from, to) {
    let outputPath = (this.streamURL + from + this.streamKey)
    let fileListAll = fs.readdirSync(videoFolderPath)
    let fileList = fileListAll.slice(from, to)
    ...
    return new Promise((resolve, reject) => {
      ffmpegCLI.on('start', (commandLine) => {
        ...
        .on('progress', (progress) => {
          logger.trace(`Video ${videoFilePath} processing: ${progress.percent} % done`)
        })
        // .on('stderr', (stderrLine) => {
        //   logger.trace(stderrLine)
        // })
  ...
  async copy (InputStream) {
    ...
    return new Promise((resolve, reject) => {
      ffmpegCLI.on('start', (commandLine) => {
        ...
        .on('progress', (progress) => {
          logger.trace(`Video ${InputStream} processing: ${progress.percent} % done`)
        })
        .on('stderr', (stderrLine) => {
          logger.trace(stderrLine)
        })
